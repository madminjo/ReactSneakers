...
const App = () => {
		...
		async function fetchData() {
			try {
				const [cartResponse, favoritesResponse, itemsResponse, ordersResponse] =
					await Promise.all([
						axios.get(
							'https://68e6896e21dd31f22cc61323.mockapi.io/api/v1/cart'
						),
						axios.get(
							'https://68e234bd8943bf6bb3c5ec03.mockapi.io/api/v1/favorites'
						),
						axios.get(
							'https://68e6896e21dd31f22cc61323.mockapi.io/api/v1/users'
						),
						axios.get(
							'https://68e234bd8943bf6bb3c5ec03.mockapi.io/api/v1/orders'
						),
					])
	...
	// Блокируем прокрутку фона, когда открыт drawer
	useEffect(() => {
	...
	return (
		<AppContext.Provider
			value={{ items, cartItems, favorites, isItemAdded, numbers, setNumbers }}
		>
			<div className='clear mx-auto max-w-[1080px] my-6 sm:my-12 px-3 sm:px-6 bg-white rounded-2xl shadow-[0_10px_20px_rgba(0,0,0,0.04)]'>
				{cartOpened && (
					<Drawer
						items={cartItems}
